...
def flavour_from_request(request):
    if hasattr(request, 'flavour'):
        return request.flavour

    if hasattr(request, 'session'):
        try:
            return request.session[settings.FLAVOURS_SESSION_NAME]
        except KeyError:
            pass

    try:
        return request.COOKIES[settings.FLAVOURS_COOKIE_NAME]
    except KeyError:
        pass
...
def check_for_flavour(flavour):
    return flavour in settings.FLAVOURS


def templates_for_flavour(request, templates):
    ...
    for template in templates:
        if flavour:
            flavoured_template = u'%s%s/%s' % (settings.FLAVOURS_TEMPLATE_PREFIX, flavour, template)
            flavour_templates.append(flavoured_template)
        flavour_templates.append(template)
    return flavour_templates
